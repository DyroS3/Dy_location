...
function OpenVehicleMenu() -- Menu location véhicules
    ...
    local elements = {
        {label = '使用-摩托艇', value = 'seashark', price = 500},
        {label = '使用-冲浪板', value = 'surfboard', price = 500}
    }
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            for i=1, #elements, 1 do
                ...
                TriggerServerEvent('dy_location:Buy', price) -- Event permetant de faire payer le joueur
                break
            end
            menu.close()
    end,
...
AddEventHandler('dy_location:hasEnteredMarker', function(zone)

    if zone == 'LocationVehicleEntering' then
        CurrentAction     = 'locationVehicle_menu'
        CurrentActionMsg  = '键下 [E] 进行冲浪'
        CurrentActionData = {}
	end

end)

AddEventHandler('dy_location:hasExitedMarker', function(zone)
    CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('dy_location:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('dy_location:hasExitedMarker', LastZone)
